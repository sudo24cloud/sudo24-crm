...
export default function Attendance() {
  ...
  const [employeeId, setEmployeeId] = useState(""); // optional filter for admin
  ...
  const loadToday = async () => {
    try {
      const res = await api.get(`${ATT_URL}/me/today`);
  ...
  /** ✅ Post helper: sends photoBase64 + lat/lng/accuracy */
  const postAction = async (path, successMsg, stopAfter = false) => {
    setMsg("");
    ...
    setLoading(true);
    try {
      const res = await api.post(`${ATT_URL}${path}`, {
        photoBase64: p.photoBase64,
        lat: p.geo.lat,
        lng: p.geo.lng,
        accuracy: p.geo.accuracy,
      });

      setToday(res?.data || null);
      setMsg(successMsg);
      if (stopAfter) stopCamera();
    } catch (e) {
      setMsg(e?.response?.data?.message || `❌ ${successMsg.replace("✅ ", "")} failed`);
    } finally {
      setLoading(false);
    }
  };

  /** ✅ Correct endpoints (backend routes) */
  const doCheckIn = () => postAction("/me/checkin", "✅ Checked-in");
  const doBreakStart = () => postAction("/me/break/start", "✅ Break started");
  const doBreakEnd = () => postAction("/me/break/end", "✅ Break ended");
  const doCheckOut = () => postAction("/me/checkout", "✅ Checked-out", true);
  ...
  const computedNetMin = useMemo(
    () => Math.max(0, computedTotalMin - computedBreakMin),
  ...
  /** ✅ Reports: /me/report + /admin/report */
  const loadReport = async () => {
    setMsg("");
    setLoading(true);
    try {
      const my = await api.get(`${ATT_URL}/me/report?from=${from}&to=${to}`);
      setRows(my?.data || []);

      if (isAdminOrManager) {
        const q = employeeId ? `&userId=${encodeURIComponent(employeeId)}` : "";
        const ad = await api.get(`${ATT_URL}/admin/report?from=${from}&to=${to}${q}`);
        setAdminRows(ad?.data || []);
      }
    } catch (e) {
    ...
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div style={cardStyle()}>
        ...
        >
          ...
          <div style={cardStyle()}>
            ...
            <div style={{ marginTop: 6, display: "grid", gap: 6 }}>
              <div>
                Gross: <b>{fmtClock(runningWorkMs)}</b>
              </div>
              <div>
                Break: <b>{fmtClock(runningBreakMs)}</b>
              </div>
              <div>
                Net: <b>{fmtMin(computedNetMin)}</b>
              </div>
            </div>
          </div>
          ...
          <div style={cardStyle()}>
            ...
            {geo ? (
              <div style={{ marginTop: 6, fontSize: 12 }}>
                Lat: <b>{geo.lat.toFixed(6)}</b>
                <br />
                Lng: <b>{geo.lng.toFixed(6)}</b>
                <br />
            ...
            <div style={{ marginTop: 10, display: "flex", gap: 10, flexWrap: "wrap" }}>
              <button onClick={fetchLocation} style={btnStyle(false)}>
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button onClick={startCamera} style={btnStyle(true)}>
            📷 Start Camera
          </button>
          <button onClick={toggleFacing} style={btnStyle(false)}>
            🔁 Switch
          </button>
          <button onClick={capture} style={btnStyle(false)} disabled={!cameraOn}>
            📸 Capture
          </button>

          <div style={{ marginLeft: "auto", display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button onClick={doCheckIn} style={btnStyle(true)} disabled={loading}>
              ✅ Check-in
            </button>
            <button onClick={doBreakStart} style={btnStyle(false)} disabled={loading}>
              ⏸ Break Start
            </button>
            <button onClick={doBreakEnd} style={btnStyle(false)} disabled={loading}>
              ▶ Break End
            </button>
            <button onClick={doCheckOut} style={btnStyle(true)} disabled={loading}>
              🚪 Check-out
            </button>
          </div>
        </div>
        ...
        >
          <div style={{ border: "1px solid #e5e7eb", borderRadius: 16, padding: 10 }}>
            ...
            <div style={{ marginTop: 10, display: "flex", gap: 10 }}>
              <button onClick={stopCamera} style={btnStyle(false)}>
                ⛔ Stop
              </button>
            </div>
          </div>
          ...
          <div style={{ border: "1px solid #e5e7eb", borderRadius: 16, padding: 10 }}>
            ...
            {snapshot ? (
              <img src={snapshot} alt="proof" style={{ width: "100%", borderRadius: 12 }} />
            ) : (
              <div style={{ color: "#6b7280" }}>No photo captured.</div>
            )}
          </div>
        </div>
      ...
      <div style={cardStyle()}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button onClick={() => setPreset("day")} style={btnStyle(false)}>
              Daily
            </button>
            <button onClick={() => setPreset("week")} style={btnStyle(false)}>
              Weekly
            </button>
            <button onClick={() => setPreset("month")} style={btnStyle(false)}>
              Monthly
            </button>
          </div>
        </div>
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          <label style={{ fontSize: 12, color: "#6b7280" }}>
            ...
            <input
              ...
              type="date"
              style={{ marginLeft: 8, height: 40, borderRadius: 12, border: "1px solid #e5e7eb", padding: "0 10px" }}
            />
          </label>
          <label style={{ fontSize: 12, color: "#6b7280" }}>
            To
          ...
          <button onClick={loadReport} style={btnStyle(true)} disabled={loading}>
        ...
        <div style={{ marginTop: 12, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
          <div style={cardStyle()}>
            <div style={{ fontSize: 12, color: "#6b7280" }}>Present Days</div>
            <div style={{ fontWeight: 900, marginTop: 6 }}>{reportStats.present}</div>
          </div>
          <div style={cardStyle()}>
            <div style={{ fontSize: 12, color: "#6b7280" }}>Full Days</div>
            <div style={{ fontWeight: 900, marginTop: 6 }}>{reportStats.fullDay}</div>
          </div>
          <div style={cardStyle()}>
            <div style={{ fontSize: 12, color: "#6b7280" }}>Total Net Time</div>
            <div style={{ fontWeight: 900, marginTop: 6 }}>{fmtMin(reportStats.totalNet)}</div>
          </div>
          <div style={cardStyle()}>
            <div style={{ fontSize: 12, color: "#6b7280" }}>Total Break</div>
            <div style={{ fontWeight: 900, marginTop: 6 }}>{fmtMin(reportStats.totalBreak)}</div>
          </div>
        </div>
          ...
          {listToShow.map((r) => (
            <div key={r._id} style={{ border: "1px solid #e5e7eb", borderRadius: 16, padding: 12 }}>
              ...
              <div style={{ marginTop: 8, display: "flex", gap: 16, flexWrap: "wrap", fontSize: 12, color: "#374151" }}>
                <div>
                  Check-in: <b>{r.checkInAt ? new Date(r.checkInAt).toLocaleTimeString() : "-"}</b>
                </div>
                <div>
                  Check-out: <b>{r.checkOutAt ? new Date(r.checkOutAt).toLocaleTimeString() : "-"}</b>
                </div>
                <div>
                  Total: <b>{fmtMin(r.totalMin || 0)}</b>
                </div>
                <div>
                  Break: <b>{fmtMin(r.breakMin || 0)}</b>
                </div>
                <div>
                  Net: <b>{fmtMin(r.netWorkMin || 0)}</b>
                </div>
              </div>
            </div>
          ))}
